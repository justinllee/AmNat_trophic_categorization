### Script objectives:
### Visualize dietary niche states estimated from non-phylogenetic implementation
...
##-----Part 1: Generate barplots for Non-phylogenetic Results ------

## Traditional
trad_states <- read.csv("/PATH/Nonphylo BHC/state_proportions_prey_traditional.csv")
trad_states <- trad_states[1:11,2:14]
trad_states <- t(trad_states)
trad_states <- apply(trad_states, 2, function(x){x*100/sum(x,na.rm=T)}) #converts to percent

state_order <- c("amphibians", "frogs", "caecilians", "lizards", 
                 "reptile_eggs", "amphisbaenians", "snakes", "birds", 
                 "bird_eggs", "mammals", "fish", "invertebrates", "other")
names(state_order) <- c("indianred2", "indianred", "indianred4", 
                        "deepskyblue", "turquoise", "cyan1", "seagreen3", 
                        "darkolivegreen2", "darkolivegreen", "royalblue3", 
                        "mediumpurple", "goldenrod1", "gray")
...
arthropoda <- arthropoda(11)
names(arthropoda) <- c(arthropods, "Arthropoda", "Insecta", "Chilopoda")
...
phylo1colors <- c(actinopterygii, amphibia, mammalia, squamata, 
                  serpentes, testudines, aves, onychophora, arthropoda, 
                  annelid, mollusca, other)

phylo1_states <- read.csv("/PATH/Nonphylo BHC/state_proportions_prey_phylo1.csv")
phylo1_states <- phylo1_states[1:9,2:65]
...
onychophora <- c("lightgoldenrod2", "lightgoldenrod")
names(onychophora) <- c("Euonychophora", "Peripatidae")

arthropoda <- colorRampPalette(c("goldenrod4", "goldenrod1"))
arthropoda <- arthropoda(12)
names(arthropoda) <- c(arthropods, "Diptera", "Coleoptera", "Arthropoda",
                       "Insecta", "Chilopoda")
...
phylo2_states <- read.csv("/PATH/Nonphylo BHC/state_proportions_prey_phylo2.csv")
phylo2_states <- phylo2_states[1:6,2:144]
...
## Functional
functional_states <- read.csv("/PATH/Nonphylo BHC/state_proportions_prey_functional1.csv")
functional_states <- functional_states[1:10,2:12]
...
## Habitat
habitat_states <- read.csv("/PATH/Nonphylo BHC/state_proportions_prey_functional2.csv")
habitat_states <- habitat_states[1:8,2:6]
habitat_states <- t(habitat_states)
habitat_states <- apply(habitat_states, 2, function(x){x*100/sum(x,na.rm=T)}) #converts to percent

habitat_order <- c("fossorial", "terrestrial", "aquatic", 
                   "marine", "arboreal")
...
## MBT Prey Type
mbt_states <- read.csv("/PATH/Nonphylo BHC/state_proportions_prey_type.csv")
mbt_states <- mbt_states[1:8,2:5]
...
## Plot
plot.new()
pdf(height = 9, width = 12, file = "Nonphylo_BHC_dietstates_figure.pdf")
par(mfrow = c(2,3), 
    oma = c(1,1,1,1), 
    mar = c(3,6,6,6),
    cex.axis = 0.75,
...
barplot(as.matrix(trad_states), col = names(state_order), 
        border = NA, cex.axis = 1.25, cex.names = 0.9, cex.lab = 1.25,
...
title(expression(paste("Traditional ", bold("[T1]"))), line = 0.75)

barplot(as.matrix(phylo1_states), col = phylo1colors, 
        border = NA, cex.axis = 1.25, cex.names = 1, cex.lab = 1.25,
        ylim = c(0,100),
        ylab = "Proportion of Diet (%)", mgp=c(2.25,0.5,0), 
        axes = TRUE, las = 1,)
title(expression(paste("Order ", bold("[T2]"))), line = 0.75)
...
barplot(as.matrix(phylo2_states), col = phylo2colors, 
        ...
        ylim = c(0,100),
        axes = TRUE, las = 1)
title(expression(paste("Family ", bold("[T3]"))), line = 0.75)
...
title(expression(paste("Functional ", bold("[E1]"))), line = 0.75)
...
title(expression(paste("Habitat ", bold("[E2]"))), line = 0.75)

barplot(as.matrix(mbt_states), col = names(mbt_order), 
        border = NA, cex.axis = 1.25, cex.names = 1, cex.lab = 1.25,
        ylab = "Proportion of Diet (%)", mgp=c(2.25,0.5,0), 
        ylim = c(0,100),
        axes = TRUE, las = 1,
        legend.text = c("Type 1", "Type 2", "Type 3", "Type 4"),
        args.legend = list(bty = "n", inset = -0.2, x = "right", 
                           ncol = 1, cex = 0.8, border = NA))
title(expression(paste("MBT Prey Type ", bold("[E3]"))), line = 0.75)
dev.off()
